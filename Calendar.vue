<template>
  <div class="calendar-box">
    ...
    <div class="calendar-list">
      <ul>
        <li v-for="(item,index) in weekDate" :key="item.date" :ref="'week'+index" @click="weekClick(index)">
...
export default {
  ...
  methods: {
    ...
    weekChange(val) {
      ...
      const startDay = new Date(val).getTime() - 3600000 * (day - 1) * 24
      const endDay = new Date(val).getTime() + 3600000 * (7 - day) * 24
      this.rangeDate[0] = parseTime(startDay, 'y-m-d')
      this.rangeDate[1] = parseTime(endDay, 'y-m-d')
      for (let i = 0; i < 7; i++) {
        this.weekDate.push({
          week: this.week[i],
          date: parseTime(startDay + 3600000 * i * 24, 'm-d')
        })
      }
    },
    weekBtn(type) {
